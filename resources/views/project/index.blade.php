...
<section class="content">
    <div class="container-fluid">
        <div class="row mb-2">
            @if($role == '2' || $employee->reporting_person == 1 || $check_department->department == '1')
                    ...
                    <!-- View Project Modal -->
                                                        ...
                                                        <div class="card card-primary card-outline card-tabs">
                                                            <div class="card-header p-0 pt-1 border-bottom-0">
                                                                <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
                                                                    ...
                                                                    <li class="nav-item" id="database-tab">
                                                                        <a class="nav-link" data-toggle="pill" href="#database" role="tab" aria-controls="database" aria-selected="false">Database</a>
                                                                    </li>
                                                                    <li class="nav-item" id="domain_hosting-tab">
                                                                        <a class="nav-link" data-toggle="pill" href="#domain_hosting" role="tab" aria-controls="domain_hosting" aria-selected="false">Domain Host</a>
                                                                    </li>
                                                                    <li class="nav-item">
                     ...
                     <div class="card-body">
                        <div class="CustomFilter">
                            @if($role == '2' || $employee->reporting_person == 1 || $check_department->department == 1)
                            <select id="TeamFilter" class="form-control form-control-sm"  style="width:25%">
                               <option value="">All upwork profile</option>
                                @foreach($upworkValues as $profilename)
                                <option>{{ $profilename }}</option>
                                @endforeach    
                            </select>
                            <select id="AssEmpValues" class="form-control form-control-sm"  style="width:25%">
                               <option value="">All Assign Employee</option>
                                @foreach($ass_empValues as $profilename1)
                                <option>{{ $profilename1 }}</option>
                                @endforeach    
                            </select>
                       ...
                       <table id="project_tbl" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @foreach($project as $proj)
                                    @php
                                        $ex_timer_id = explode(',', $proj->timer_id ?? '');
                                        $ex_emp_id = explode(',', $proj->employee_id ?? '');
                                    @endphp

                                    @if(in_array($userid, $ex_emp_id) || $role == 2 || $employee->reporting_person == 1 || $check_department->department == 1)
                                        <tr>
                                            <td>{{ $proj->project_name }}</td>

                                            <td>
                                                @if(!empty($proj->staging_url))
                                                    <p>
                                                        <b>Staging: </b>
                                                        <a href="{{ $proj->staging_url }}" target="_blank">{{ $proj->staging_url }}</a>
                                                    </p>
                                                @endif
                                            ...
                                            <td>
                                                @foreach($ex_timer_id as $id)
                                                    @php
                                                        $emp = \App\Models\Employee::find($id);
                                                    @endphp
                                                    @if($emp)
                                                        <span class="btn btn-info">{{ $emp->first_name }}</span>
                                                    @endif
                                                @endforeach
                                            </td>
                                            ...
                                            <td>{{ $proj->created_at->toDateString() }}</td>

                                            <td>
                                                <a href="javascript:void(0)" class="btn btn-info project-view" data-id="{{ $proj->id }}">
                                            ...
                                            @if($role == 2 || $employee->reporting_person == 1 || $check_department->department == 1)
                                                <td>
                                                    <button onclick="location.href='{{ route('project.edit', [$proj->id]) }}'" class="btn btn-secondary">
...
<script>
jQuery(function ($) {
    var reporting_person = "<?php echo $employee->reporting_person ?>";
    var role = "<?php echo $role ?>";
    var check_department = "<?php echo $check_department->department ?? '' ?>";
    ...
    $("#project_tbl").dataTable({
       ...
       "pageLength": 10,
       "searchable": true,
       "order": (role === '2' || reporting_person === 1 || check_department === '1') ? [[4, 'desc']] : [[3, 'desc']]  
        
    });

    
    var tables = $('#project_tbl').DataTable();
   
    if(role == '2' || reporting_person == 1 || check_department == '1'){
        //specific column upwork profile
        $("#project_tbl.dataTables_filter").append($("#TeamFilter"));
        var TeamIndex1 = 0;
        $("#project_tbl th").each(function (i) {
            if ($($(this)).html() == "Upwork Profiles") {
                TeamIndex1 = i; 
                return false;
            }
        });
        ...
        $.fn.dataTable.ext.search.push(
            function (settings, data, dataIndex) {
                var selectedItem1 = $('#TeamFilter').val()
                var Team1 = data[TeamIndex1];
                if (selectedItem1 === "" || Team1.includes(selectedItem1)) {
                    return true;
                }
                return false;
            }
        ...
        $("#project_tbl.dataTables_filter").append($("#AssEmpValues"));
        var TeamIndex = 0;
        $("#project_tbl th").each(function (i) {
            if ($($(this)).html() == "Assign Employee") {
                TeamIndex = i; 
                return false;
            }
        });
        ...
        $.fn.dataTable.ext.search.push(
            function (settings, data, dataIndex) {
                var selectedItem = $('#AssEmpValues').val()
        ...
        $("#AssEmpValues").change(function (e) {
            tables.draw();
        });
    }      
             ...
             $.ajax({
                ...
                success: function (response) {
                    ...
                    $.each(timers, function(index, value){
                        ...
                        if(value.upwork_username == null){
                            var first_name = '';
                        }else{
                            var first_name = '<a  class="copy_ftp" data-id="'+index+'" data-type="upwork_username" href="javascript:void(0)"><i class="fa fa-copy"></i></a><span style="color:#32CD32; margin-left:10px"></span>';
                        }
                        ...
                        upwork_password_details += '<div class="row"><div class="col-2"><label>Username: </label></div><div class="col-10"><p>'+value.upwork_username+''+first_name+'</p></div></div><div class="row"><div class="col-2"><label>Password: </label></div><div class="col-10"><p>'+u_password+upwork_password+'</p></div></div><hr>';
                    });
